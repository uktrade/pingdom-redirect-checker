    ...
    def handle(self, *args, **options):
        ...
        for current_url in Urllist.objects.filter(enable=True):
            ...
            if response_url == current_url.target_url:
                Urllist.objects.filter(id=current_url.id).update(broken_redirect=False,
                                                                    actual_target=None)
            else:
                print ("Actual url: ", response_url)
                print ("Error redirect broken")
                Urllist.objects.filter(id=current_url.id).update(broken_redirect=True,
                                                                    actual_target=response_url)
        t1 = time.time()
        total_time = (t1 - t0) * 1000
        Responsetime.objects.update_or_create(id=1,
